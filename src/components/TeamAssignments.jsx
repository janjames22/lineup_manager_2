...
export default function TeamAssignments({ musicians, onChange, readOnly = false }) {
  if (readOnly) {
    ...
    return (
      <div className="print-team-grid grid min-w-0 gap-3 sm:grid-cols-2 lg:grid-cols-3">
        {filled.map(([key, label]) => (
          <div key={key} className="print-team-card min-w-0 rounded-xl border border-slate-800 bg-slate-950 p-4 shadow-inner">
            <p className="print-accent text-[10px] font-black uppercase tracking-widest text-blue-400">{label}</p>
            <p className="mt-1 break-words font-bold text-white">{musicians[key]}</p>
          </div>
        ))}
      </div>
    );
